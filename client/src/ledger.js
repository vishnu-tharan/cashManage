...
export const amountOf = (value, currency) => {
  ...
  if (
    ...
    Math.abs(amount - Math.round(amount)) > 0.00001 ||
    !Number.isSafeInteger(Math.round(amount)) ||
    amount <= 0
  )
    throw new Error(
      "Enter a positive amount with the correct currency precision",
    );
...
export function record(before, next, label = "Workspace updated") {
  ...
  const changes = [];
  for (const id of new Set([...previous.keys(), ...after.keys()]))
    if (JSON.stringify(previous.get(id)) !== JSON.stringify(after.get(id)))
      changes.push({
        id,
  ...
  const result = {
    ...upgrade(next),
    history: [
      { id: uid(), at: new Date().toISOString(), label, changes },
      ...(next.history || before.history || []),
    ].slice(0, 200),
  };
  while (result.history.length && JSON.stringify(result).length > 1800000)
    result.history.pop();
  return result;
}
export function undo(data, event) {
  if (!event.changes.length)
    throw new Error("This event has no transaction changes to undo");
  let ts = [...data.transactions];
  for (const c of event.changes) {
    const current = ts.find((t) => t.id === c.id) || null;
    if (JSON.stringify(current) !== JSON.stringify(c.after))
      throw new Error(
        "A transaction changed later. Review its current value before undoing.",
      );
  ...
  for (const change of event.changes)
    if (!change.before && change.after?.debt && !change.after.repayment) {
      const id = change.after.debt;
      if (ts.some((t) => t.debt === id))
        throw new Error("Undo the repayments before undoing this debt");
      debts = debts.filter((d) => d.id !== id);
    }
  return { ...data, debts, transactions: ts };
}
export function transfer(data, input) {
  const source = data.books.find((b) => b.id === input.source),
    destination = data.books.find((b) => b.id === input.destination);
  if (!source || !destination || source.id === destination.id)
    throw new Error("Choose two different cashbooks");
  const amount = amountOf(input.amount, source.currency);
  const received =
    source.currency === destination.currency
      ? amount
      : amountOf(input.received, destination.currency);
  const id = uid();
  const base = {
    date: input.date,
    category: "Transfer",
  ...
  return {
    ...
    transactions: [
      ...data.transactions,
      { ...base, id: uid(), book: source.id, type: "expense", amount },
      {
        ...base,
        id: uid(),
        book: destination.id,
...
export function postRecurring(data, date) {
  ...
  return { data: { ...data, transactions: ts, recurring }, count };
}
export function debtRemaining(data, debt) {
  return (
    debt.amount -
    data.transactions
      .filter((t) => t.debt === debt.id && t.repayment)
      .reduce((s, t) => s + t.amount, 0)
  );
}
export function createDebt(data, f) {
  const book = data.books.find((b) => b.id === f.book),
    amount = amountOf(f.amount, book.currency),
    id = uid();
  return {
    ...data,
    debts: [
      ...(data.debts || []),
      { ...f, id, amount, currency: book.currency },
    ],
    transactions: [
      ...data.transactions,
      {
...
export function repay(data, debt, value, date) {
  ...
  if (amount > debtRemaining(data, debt))
    throw new Error("Repayment exceeds the remaining debt");
  return {
    ...data,
    transactions: [
      ...data.transactions,
...
export function prepareImport(data, rows, bookId, mapping = {}) {
  ...
  rows.forEach((r, index) => {
    try {
      ...
      const rawAmount = String(read("amount") ?? "")
        .replace(/,/g, "")
        .trim();
      const n = Number(rawAmount);
      if (!rawAmount || !Number.isFinite(n)) throw new Error("Invalid amount");
      const rawType = String(read("type") || "").toLowerCase();
      if (rawType.startsWith("transfer"))
        throw new Error(
          "Recreate wallet transfers through Planning to preserve both sides",
        );
      if (
        rawType &&
        ![
        ...
        throw new Error(
          "Unknown direction; map credit/debit values to income/expense",
        );
      const type = ["income", "credit", "in"].includes(rawType)
        ? "income"
        : ["expense", "outcome", "debit", "out"].includes(rawType)
          ? "expense"
          : n < 0
            ? "expense"
            : "income";
      ...
      if (read("currency") && read("currency") !== book.currency)
        throw new Error("Currency differs from destination wallet");
      const t = {
        id: uid(),
        book: bookId,
        date,
        ...
        note: String(read("note") || r.remark || "Imported entry").slice(
          0,
          200,
        ),
  ...
  return data.books
    ...
    .map((b) => {
      const spent = data.transactions
        ...
        .reduce((s, t) => s + t.amount, 0);
      return { ...b, spent, percent: Math.round((spent / b.budget) * 100) };
    })
...
export function mergeVersions(local, remote, choices = {}) {
  ...
  for (const field of [
    ...
    for (const id of map.keys())
      if (choices[`${field}:${id}`] === "delete") map.delete(id);
