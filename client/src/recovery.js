...
export async function recoverAccount() {
  const username = (await ask("Account username:"))?.toLowerCase().trim(),
    key = (await ask("Recovery key from your saved file:"))?.trim();
  if (!username || !key) throw new Error("Recovery cancelled");
  const recoveryProof = await digest(key);
  const r = await api("/auth/recovery", "POST", { username, recoveryProof });
  ...
  const envelope = await decrypt(
    r.envelope,
    await derive(key, r.envelope.salt),
  );
  ...
  localStorage.setItem(
    `cm:${username}`,
    JSON.stringify({ vault, revision: r.revision + 1, dirty: false }),
  );
