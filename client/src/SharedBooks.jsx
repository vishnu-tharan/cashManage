...
import { api, derive, seal, unseal, randomSalt, initial, money, today, currencies } from "./storage";
...
export default function SharedBooks({
  ...
  return <section className="card">
      ...
      act(async () => {
        ...
        const key = await derive(f.password, salt);
        const data = initial(f.name);
        data.books = [{
          ...data.books[0],
          name: f.name,
          currency: f.currency
        ...
        <input name="name" aria-label="Shared book name" placeholder="Family spending" maxLength={50} required />
        <input name="password" aria-label="Shared book passphrase" type="password" placeholder="Separate shared passphrase" minLength={12} required />
        <select name="currency" aria-label="Shared book currency">
          {currencies.map(c => <option key={c}>{c}</option>)}
        </select>
        <button disabled={session.guest}>Create shared book</button>
      </form>
      {list.map(b => <div className="rate" key={b.id}>
          <span>
            {b.name} · {b.role}
          </span>
        ...
        const key = await derive(password, r.vault.salt);
        setOpened({
          ...r,
          key,
          data: await unseal(r.vault, key)
        });
      })}>
            Unlock
          </button>
        </div>)}
      {opened && <div className="shared-content">
          ...
          {opened.data.transactions.map(t => <div className="rate" key={t.id}>
              <span>
                {t.date} · {t.note}
              </span>
          ...
          {opened.role !== "viewer" && <form className="inline-form" onSubmit={e => {
        e.preventDefault();
        const f = Object.fromEntries(new FormData(e.target));
        act(async () => {
          const data = {
            ...opened.data,
            transactions: [...opened.data.transactions, {
              id: uid(),
              book: opened.data.books[0].id,
              type: f.type,
              date: today(),
              amount: amountOf(f.amount, opened.data.books[0].currency),
              category: "Other",
              note: f.note,
          ...
          e.target.reset();
              ...
              <input name="amount" aria-label="Shared amount" type="number" min="0.01" step="0.01" required />
              <input name="note" aria-label="Shared description" placeholder="Description" maxLength={200} required />
              <button>Add shared entry</button>
            </form>}
          <h3>Members</h3>
          {opened.members.map(m => <p key={m.username}>
              {m.username} · {m.role}
            </p>)}
          {opened.role === "owner" && <form className="inline-form" onSubmit={e => {
        e.preventDefault();
        const f = Object.fromEntries(new FormData(e.target));
        act(async () => {
          ...
          setOpened({
            ...
            members: r.members
              ...
              <input name="username" placeholder="Existing username" aria-label="Member username" required pattern="[a-z0-9_]{3,24}" />
