...
export default function AccountSecurity({
  ...
  const credentials = async () => {
    ...
    return {
      ...
      code: (await ask("Authenticator code (leave empty if MFA is disabled):")) || ""
    };
  };
  return <section className="card">
      ...
      <button disabled={session.guest} onClick={async () => act(async () => setState(await api("/security")))}>
      ...
      <div className="settings-actions">
        <button disabled={session.guest} onClick={async () => act(async () => {
        const c = await credentials();
        setSetup(await api("/security/mfa/setup", "POST", c));
      })}>
          Set up authenticator
        </button>
        {setup && <div className="notice">
            <div>
              <p>
        ...
        <button disabled={session.guest} onClick={async () => act(async () => {
        const c = await credentials();
        const recoveryKey = hex(crypto.getRandomValues(new Uint8Array(32)));
        const salt = randomSalt();
        const raw = hex(await crypto.subtle.exportKey("raw", session.key));
        const envelope = await seal({
          raw,
          salt: session.salt
        }, await derive(recoveryKey, salt), salt);
        await api("/security/recovery", "POST", {
          ...c,
          recoveryProof: await digest(recoveryKey),
          envelope
        });
        download("cashmanage-recovery-key.txt", `CashManage recovery key\nUsername: ${session.username}\nKey: ${recoveryKey}\nKeep this key offline. It can reset your password and MFA.\n`, "text/plain");
        setState(await api("/security"));
      })}>
          Generate recovery key file
        </button>
        <button disabled={session.guest} onClick={async () => act(async () => {
        if (!(await confirmAction("Changing password signs out all devices and invalidates the current recovery key. Continue?"))) return;
        const c = await credentials();
        ...
        const saved = JSON.parse(localStorage.getItem(`cm:${session.username}`));
        if (saved.dirty) throw new Error("Sync your changes before changing password");
        const salt = randomSalt();
        ...
        localStorage.setItem(`cm:${session.username}`, JSON.stringify({
          vault,
